...
class ZEDStreamer:
    ...
    def start_video_writer(self, output_path: str, codec: str = "mp4v", stereo: bool = True):
        """Start video recording. If stereo=True (default), records side-by-side left|right."""
        fourcc = cv2.VideoWriter_fourcc(*codec)
        w, h = (2 * self.width, self.height) if stereo else (self.width, self.height)
        self.video_writer = cv2.VideoWriter(output_path, fourcc, self.fps, (w, h))
        print(f"Recording to: {output_path} ({w}x{h} {'stereo' if stereo else 'left only'})")

    def grab_frame(self) -> Optional[dict]:
        """Grab a single frame from ZED camera (left and right eyes).

        Returns:
            Dictionary with 'image_left', 'image_right', 'image' (same as left for compat),
            'timestamp', and optionally 'depth'
        """
        status = self.zed.grab(self.runtime_params)
        if status != sl.ERROR_CODE.SUCCESS:
            return None

        # Left eye: retrieve and convert BGRA -> RGB
        self.zed.retrieve_image(self.image, sl.VIEW.LEFT)
        left_data = self.image.get_data()
        image_left = left_data[:, :, :3][:, :, ::-1].copy()

        # Right eye: reuse sl.Mat by retrieving RIGHT view
        self.zed.retrieve_image(self.image, sl.VIEW.RIGHT)
        right_data = self.image.get_data()
        image_right = right_data[:, :, :3][:, :, ::-1].copy()

        result = {
            "image_left": image_left,
            "image_right": image_right,
            "image": image_left,  # backward compat
            "timestamp": time.time(),
        }
    ...
    def publish_frame(self, frame: dict):
        """Publish frame over ZMQ (includes image_left and image_right)."""
    ...
    def run(self, display: bool = False, duration: Optional[float] = None):
        ...
        try:
            while self.running:
                ...
                # Write to video file (side-by-side left | right)
                if self.video_writer:
                    bgr_left = frame["image_left"][:, :, ::-1]
                    bgr_right = frame["image_right"][:, :, ::-1]
                    side_by_side = np.hstack([bgr_left, bgr_right])
                    self.video_writer.write(side_by_side)

                # Display preview: separate left and right windows
                if display:
                    bgr_left = frame["image_left"][:, :, ::-1]
                    bgr_right = frame["image_right"][:, :, ::-1]
                    cv2.imshow("ZED Left", bgr_left)
                    cv2.imshow("ZED Right", bgr_right)
